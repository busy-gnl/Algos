...
(async () => {
  ...
  for (const folder of folders) {
    ...
    if (answer && typeof answer === "function") {
      ...
      const output = (await import(`./challenges/${folder}/output.js`))
        ?.default;
      ...
      try {
        ...
        if (deepEqual(output, result) === true) {
          ...
          console.log(
            `⌛ Time (machine dependant): ${diff[0] * NS_PER_SEC + diff[1]}ns`
          );
          console.log(
            `🏅 Number of chars (after tranpile): ${
          ...
          console.log(
            `❌ Too bad, your result does not match the expected output! Try again! ❌`
          );
        }
      } catch (e: any) {
        console.error(
          `🔥 Uh oh, something went wrong! See exception bellow: 🔥`
        );
        console.error(e);
        e.printStackTrace();
      }
  ...
  if (challenges === 0) {
    console.warn(
      `🤔 No challenge executed, uncomment export function in ./src/challenges/X/answer.ts file to try solving some of them!`
    );
  }
})();
  ...
  if (typeof expected !== typeof submitted) {
    ...
    if (Array.isArray(expected) === true) {
      ...
      const submittedKeys = Object.keys(submitted)
        .filter((key) => submitted[key] !== undefined)
        .sort();
      if (
        deepEqual(expectedKeys, submittedKeys) === false ||
        deepEqual(
          expectedKeys.map((e) => expected[e]),
          submittedKeys.map((e) => submitted[e])
        ) === false
      ) {
        return false;
      }
